## copy from Training-Code-of-STM
## modifications: add normalization for RGB frame
import os
...
class Coco_MO_Train(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for f in range(3):
            Fs[:, f] = self.normalize(Fs[:, f])
        Ms = torch.from_numpy(self.All_to_onehot(N_masks).copy()).float()
        if num_object == 0:
            num_object += 1
        num_objects = torch.LongTensor([num_object])
        return Fs.transpose(0, 1), Ms.transpose(0, 1), num_objects, info
