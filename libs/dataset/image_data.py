...
class COCODataset(BaseData):
    ...
    def __init__(self, transform=None, sampled_frames=3, ratio=0.1) -> None:
        ...
        logger.info("Dataloder: Creating coco dataset")
        ...
        if len(COCODataset.data_items) == 0:
            self._ensure_cache()
        self.length = len(COCODataset.data_items)
    ...
    def __getitem__(self, idx):
        ...
        while num_obj==0:
            while num_obj==0:
                idx = int(random.sample(range(self.length), 1)[0])
                record = COCODataset.data_items[idx]
    ...
    def __len__(self):
        return int(self.ratio*self.length)
