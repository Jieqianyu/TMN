...
def channel_shuffle(f_r, f_m):
    assert f_r.shape == f_m.shape, 'rgb feature shape:{} != mask feature shape:{}'.format(f_r.shape, f_m.shape)
    N, C, H, W = f_r.shape
    f_r_fusion = []
    f_m_fusion = []
    for i in range(C):
        if i < C//2:
            f_r_fusion.append(f_r[:,i:i+1,:,:])
            f_r_fusion.append(f_m[:,i:i+1,:,:])
        else:
            f_m_fusion.append(f_r[:,i:i+1,:,:])
            f_m_fusion.append(f_m[:,i:i+1,:,:])

    return torch.cat(f_r_fusion, dim=1), torch.cat(f_m_fusion, dim=1)
...
class Fusion(nn.Module):
    def __init__(self, planes, opt):
        ...
        if opt.fusion_type == 'se':
            ...
            if opt.bg_type == 'branch':
                self.attention_bg = nn.Sequential(
                    nn.AdaptiveAvgPool2d(1),
                    nn.Conv2d(planes, planes, kernel_size=1),
                    nn.Sigmoid()
                    )
                logger.info('Fusion: add bg channel attention')
        ...
        elif opt.fusion_type == 'concat':
            self.type = 'concat'
            self.channel_trans = nn.Conv2d(2*planes, planes, kernel_size=1)
            logger.info('Fusion type: {}'.format(self.type))
        else:
            raise NotImplementedError

    def forward(self, f_rgb, f_mask, f_bg=None):
        ...
        if self.type == 'se':
            ...
            if f_bg is not None:
                attention_bg = self.attention_mask(f_bg)
                f_bg_after_attention = torch.mul(f_bg, attention_bg)
                f_fusion += f_bg_after_attention 
            ...
            f_concat = torch.cat([f_rgb, f_mask], dim=1)
...
class Encoder_M(nn.Module):
    def __init__(self, opt):
        ...
        resnet_rgb = getattr(models, opt.backbone)(pretrained=True)
        resnet_mask = getattr(models, opt.backbone)(pretrained=True)
        if opt.bg_type == 'branch':
            resnet_bg = getattr(models, opt.backbone)(pretrained=True)

        if opt.backbone in ['resnet18', 'resnet34']:
            r2_planes = 64
            r4_planes = 256
            r5_planes = 512
        elif opt.backbone == 'resnet50':
        ...
        elif opt.bg_type in ['conv1', 'branch']:
            self.conv1_mask = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
            self.conv1_bg = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
            logger.info('Encoder_M: add conv1 for bg')
        elif opt.bg_type == 'concat':
            self.conv1_mask = nn.Conv2d(2, 64, kernel_size=7, stride=2, padding=3, bias=False)
            logger.info('Encoder_M: 2 channel conv1 for fg and bg')
        else:
            raise NotImplementedError
        
        # fg branch
        logger.info('Encoder_M: fg branch add res1(bn, relu, maxpool)')
        self.bn1_mask = resnet_mask.bn1
        self.relu_mask = resnet_mask.relu  # 1/2
        self.maxpool_mask = resnet_mask.maxpool
        logger.info('Encoder_M: fg branch add res2(layer1)')
        self.res2_mask = resnet_mask.layer1 # 1/4

        # bg brach
        if opt.bg_type == 'branch':
            logger.info('Encoder_M: bg branch add res1(bn, relu, maxpool)')
            self.bn1_bg = resnet_bg.bn1
            self.relu_bg = resnet_bg.relu  # 1/2
            self.maxpool_bg = resnet_bg.maxpool
            logger.info('Encoder_M: bg branch add res2(layer1)')
            self.res2_bg = resnet_bg.layer1 # 1/4

        # fusion 
        if opt.fusion_type == 'shuffle':
            logger.info('fusion type: shuffle')
            self.shuffle = True
        else:
            logger.info('Encoder_M: add first fusion block')
        ...
        # iou prediction
        self.pred_layer = opt.pred_layer
        if opt.pred_layer == 'r5':
            self.pred = nn.Sequential(
                resnet_rgb.layer4, # 1/32, 2048/512
                nn.AdaptiveAvgPool2d(1),
                nn.Conv2d(r5_planes, r5_planes//2, kernel_size=1),
                nn.Conv2d(r5_planes//2, 1, kernel_size=1),
                nn.Sigmoid()
            )
            logger.info('Encoder_M: using layer 4(r5) for iou prediction')
        elif opt.pred_layer == 'r4':
            self.pred = nn.Sequential(
                nn.Conv2d(r4_planes, r4_planes//2, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(r4_planes//2, r4_planes//4, kernel_size=3, stride=1, padding=1),
                nn.AdaptiveAvgPool2d(1),
                nn.Conv2d(r4_planes//4, 1, kernel_size=1),
                nn.Sigmoid()
            )
            logger.info('Encoder_M: using layer 3(r4) for iou prediction')
        elif opt.pred_layer == 'mask':
            self.pred = nn.Sequential(
                nn.Conv2d(r2_planes, r2_planes//2, kernel_size=3, stride=1, padding=1),
                nn.Conv2d(r2_planes//2, r2_planes//4, kernel_size=3, stride=1, padding=1),
                nn.AdaptiveAvgPool2d(1),
                nn.Conv2d(r2_planes//4, 1, kernel_size=1),
                nn.Sigmoid()
            )
            logger.info('Encoder_M: using mask branch for iou prediction')
        else:
            raise NotImplementedError
    ...
    def forward(self, in_f, in_m, in_bg):
        ...
        bg = torch.unsqueeze(in_bg, dim=1).float() # add channel dim
        ...
        if self.bg_type == '':
            y = self.conv1_mask(m)
            x = x + y
        elif self.bg_type in ['conv1', 'branch']:
            y = self.conv1_mask(m)
            z = self.conv1_bg(bg)
            x = x + y + z # fusion
        elif self.bg_type == 'concat':
            y = self.conv1_mask(torch.cat([m, bg], dim=1))
            x = x + y
        else:
            raise NotImplementedError
        ...
        if self.bg_type == 'branch':
            z = self.bn1_bg(z)
            c1_z = self.relu_bg(z)
            r1_z = self.maxpool_bg(c1_z)
        ...
        ## bg
        if self.bg_type == 'branch':
            r2_z = self.res2_bg(r1_z)
            r2_x = self.fusion1(r2_x, r2_y, r2_z)
            f_m = r2_y + r2_z
        else:
            if not self.shuffle:
                r2_x = self.fusion1(r2_x, r2_y)
            else:
                r2_x, r2_y = channel_shuffle(r2_x, r2_y)
            f_m = r2_y
        ...
        # iou pred
        if self.pred_layer == 'mask':
            iou_pred = self.pred(f_m)
        elif self.pred_layer in ['r4', 'r5']:
            iou_pred = self.pred(r4_x)
        else:
            raise NotImplementedError
        n, _, _, _ = iou_pred.shape
        iou_pred = iou_pred.reshape(1, n)

        return r4_x, f_m, iou_pred
...
class STM(nn.Module):
    ...
    def memorize(self, frame, masks, num_objects): 
        ...
        r4, f_m, iou_pred = self.Encoder_M(frame_batch, mask_batch, bg_batch) # no, c, h, w
        _, c, h, w = r4.size()
        memfeat = r4
    ...
    def segment_mask(self, f_m, frame, max_obj):
        m_out = self.Decoder_M(f_m, frame)
        ...
        if self.phase == 'test':
            ...
            for idx in range(N):
                ...
                for t in range(1, T):
                    ...
                    # segment mask branch
                    f_m = features[-2]
                    m_logits = self.segment_mask(f_m, frame=frame[idx, t-1:t], max_obj=max_obj)
                    m_out = torch.softmax(m_logits, dim=1)
                    m_tmp_out.append(m_out)

                    # iou pred
                    iou_pred = torch.zeros(1, max_obj).to(key.device)
                    iou_pred[0,:num_object] = features[-1]
                    iou_tmp_out.append(iou_pred)
                ...
                iou_batch_out.append(torch.cat(iou_tmp_out, dim=0))

            batch_out = torch.stack(batch_out, dim=0) # B, T, No+1, H, W
            m_batch_out = torch.stack(m_batch_out, dim=0)
            iou_batch_out = torch.stack(iou_batch_out, dim=0) # B, T, No

            return batch_out, m_batch_out, iou_batch_out
